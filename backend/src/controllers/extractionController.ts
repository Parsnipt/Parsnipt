...
export class ExtractionController {
  ...
  static async createExtraction(
    ...
    try {
      ...
      logger.info(
        `File extracted uploaded: ${extraction.id} - ${fileName} (${fileSizeBytes} bytes)`
      );

      // Update status to processing
      ExtractionService.updateExtractionStatus(extraction.id, 'processing');

      // TODO: Issue 4 creates queue for extraction engine
      // Until Phase 4, marked as pending processing
